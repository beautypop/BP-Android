...
public class InstagramApp {
	...
	public static List<InstagramVM> mediaList = new ArrayList<InstagramVM>();
	...
	public void getMedia(final Activity activity) {
		...
		new Thread() {
			...
			public void run() {
				...
				int what = WHAT_FINALIZE;
				try {
					URL url = new URL(API_URL + "/users/self/media/recent"
							+ "/?access_token=" + mAccessToken);
					...
					for(int i=0; i<array.length(); i++){
						...
						JSONObject object2 = object1.getJSONObject("low_resolution");
						InstagramVM instagramVM = new InstagramVM();
						instagramVM.setMediaId(object.getString("id"));

						try {
							instagramVM.setCaption(object.getJSONObject("caption").getString("text"));
						}catch (Exception e){
                                e.printStackTrace();
						}
						instagramVM.setUrl(object2.getString("url"));
						mediaList.add(instagramVM);
					}

					activity.startActivity(new Intent(activity,ImportImageActivity.class));
