...
public class ThemeFragment extends TrackedFragment {
	private LinearLayout themesLayout, trendsLayout, themeImagesLayout, placeHolder;
	private int themeImageWidth, productImageWidth;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		themesLayout = (LinearLayout)view. findViewById(R.id.themesLayout);
        trendsLayout = (LinearLayout)view. findViewById(R.id.trendsLayout);
        ...
        themeImageWidth = (int) ((double) ViewUtil.getDisplayDimensions(getActivity()).width() / 3.5);  // fit around 3.5 items
        productImageWidth = (int) ((double) ViewUtil.getDisplayDimensions(getActivity()).width() / 3.0);  // fit around 3.0 items
	...
	public void showThemes(){
        ...
        if (CategoryCache.getThemeCategories().size() > 0) {
            int padding = 10;
            for (final CategoryVM vm : CategoryCache.getThemeCategories()) {
                FrameLayout layout = new FrameLayout(getActivity());
                layout.setPadding(padding, padding, padding, padding);
                layout.setLayoutParams(new ViewGroup.LayoutParams(themeImageWidth, themeImageWidth));

                ImageView imageView = new ImageView(getActivity());
                imageView.setLayoutParams(new ViewGroup.LayoutParams(themeImageWidth, themeImageWidth));
		...
		AppController.getApiService().getCategoryPopularFeed(0L, categoryVM.getId(), FeedFilter.ConditionType.ALL.name(), new Callback<List<PostVMLite>>() {
            ...
            public void success(List<PostVMLite> postVMLites, Response response) {
                for (final PostVMLite vm : postVMLites) {
                    ...
                    ImageView imageView = new ImageView(getActivity());
                    imageView.setLayoutParams(new ViewGroup.LayoutParams(productImageWidth, productImageWidth));
                    imageView.setScaleType(ImageView.ScaleType.FIT_XY);
                    ...
                    priceText.setTextColor(getActivity().getResources().getColor(R.color.light_gray));
                    priceText.setPadding(5, 0, 5, 0);
                    ...
                    if (vm.getOriginalPrice() > 0) {
                        ...
                        originalPrice.setTypeface(null, Typeface.NORMAL);
                        originalPrice.setTextColor(getActivity().getResources().getColor(R.color.gray));
                        originalPrice.setText(ViewUtil.formatPrice(vm.getOriginalPrice()));
                        originalPrice.setPadding(5, 0, 5, 0);
